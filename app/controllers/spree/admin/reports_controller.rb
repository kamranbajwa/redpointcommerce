  ...
  module Admin
    class ReportsController < Spree::Admin::BaseController
      ...
      def sales_total
        ...
        @search_customer = Order.complete.ransack(params[:q])
        @orders_customer = @search_customer.result
        ...
        @orders.each do |order|
          ...
          @totals[@orders.first.currency][:order_total] = @orders.count
          @customers_with_orders = @orders_customer.select(:email).group(:email).having("count(email) > ?", 1).count.to_a
      end
